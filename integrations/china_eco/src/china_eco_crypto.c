...
// @owner: team-B
/**
 * @file china_eco_crypto.c
 * @brief SM3 hash and SM4 block cipher for china_eco adapter.
 *
 * Extracted from china_eco_adapter.c — implements:
 * - SM3 cryptographic hash: GB/T 32905-2016 standard
...
static uint32_t sm3_p0(uint32_t x) { return x ^ sm3_rotl32(x, 9) ^ sm3_rotl32(x, 17); }
static uint32_t sm3_p1(uint32_t x) { return x ^ sm3_rotl32(x, 15) ^ sm3_rotl32(x, 23); }
static uint32_t sm3_ff0(uint32_t x, uint32_t y, uint32_t z) { return x ^ y ^ z; }
static uint32_t sm3_ff1(uint32_t x, uint32_t y, uint32_t z) { return (x & y) | (x & z) | (y & z); }
static uint32_t sm3_gg0(uint32_t x, uint32_t y, uint32_t z) { return x ^ y ^ z; }
static uint32_t sm3_gg1(uint32_t x, uint32_t y, uint32_t z) { return (x & y) | ((~x) & z); }
...
/* ================================================================== */
/*  SM4 — GB/T 32907-2016  (CBC mode, PKCS#7 padding)                 */
/* ================================================================== */
...
static uint32_t sm4_rotl32(uint32_t x, int n) { return (x << n) | (x >> (32 - n)); }

static uint32_t sm4_sub(uint32_t x)
{
    uint32_t r = 0;
    r |= ((uint32_t)SM4_SBOX[(x >> 24) & 0xFF]) << 24;
    r |= ((uint32_t)SM4_SBOX[(x >> 16) & 0xFF]) << 16;
    r |= ((uint32_t)SM4_SBOX[(x >>  8) & 0xFF]) <<  8;
    r |= ((uint32_t)SM4_SBOX[x & 0xFF]);
    return r;
}
        ...
        if (offset + CHINA_ECO_SM4_BLOCK_SIZE >= ct_size) {
            ...
            if (pad_len >= 1 && pad_len <= CHINA_ECO_SM4_BLOCK_SIZE) {
                ...
                for (size_t j = CHINA_ECO_SM4_BLOCK_SIZE - pad_len;
                    ...
                    if (decrypted[j] != pad_len) { ok = false; break; }
                }
                if (ok) { copy_size = CHINA_ECO_SM4_BLOCK_SIZE - pad_len; pad_valid = true; }
            }
        }
